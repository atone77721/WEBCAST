...
//  Writes & merges SportsWebcastPT.m3u8
// ---------------------------------------------------------
...
// 🔥 Pacific Time timestamp formatter
function formatTimestamp(ts) {
  try {
    return moment.unix(Number(ts))
      .tz("America/Los_Angeles")
      .format("MMM DD @ hh:mm A z"); // PST/PDT automatically
  } catch (e) {
    return null;
  }
}
...
async function grabM3u8FromIframe(browser, iframeUrl) {
  ...
  let filtered = [...found].filter(
    u => u.endsWith("index.m3u8") || u.includes("/tracks-v1a1/mono.ts.m3u8")
  );
...
async function main() {
  ...
  console.log(
    `✅ Found ${todayStreams.length} live/scheduled/ended games for ${now.format("MMM DD, YYYY")} PT`
  );
  ...
  fs.writeFileSync("SportsWebcastPT.m3u8", playlist, "utf-8");

  console.log(`✅ Playlist saved → SportsWebcastPT.m3u8`);
}
